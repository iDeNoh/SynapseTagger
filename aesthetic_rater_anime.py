...
def initialize_rater():
    ...
    try:
        print("Initializing ANIME rater (one-time setup)...", file=sys.stderr)
        device = "cuda" if torch.cuda.is_available() else "cpu"
        print(f"Using device: {device}", file=sys.stderr)
        model_id = "shadowlilac/aesthetic-shadow"
        processor = AutoProcessor.from_pretrained(model_id)
        model = AutoModelForImageClassification.from_pretrained(model_id).to(device)
        model.eval()
        print("Anime rater initialized and ready.", file=sys.stderr)
    except Exception as e:
        print(f"Error during anime rater initialization: {e}", file=sys.stderr)
        sys.exit(1)
...
def get_aesthetic_score(image_path):
    ...
    try:
        ...
        with torch.no_grad():
            ...
            squashed_score = torch.tanh(torch.tensor(raw_score / 2.0)).item()
